...
def register(mcp: MCPServer) -> None:
    @mcp.tool()
    @wrap_errors
    async def get_chat(ctx: Context, chat_id: int | str) -> dict:
    ...
    @mcp.tool()
    @wrap_errors
    async def get_messages(ctx: Context, chat_id: int | str, message_ids: list[int]) -> dict:
        """按 id 获取指定消息。"""
        state: ServerState = ctx.request_context.lifespan_context
        access = await access_for(ctx, state)
        chat_id = await resolve_chat_id(state.client, chat_id)
    ...
    async def search_messages(
        ctx: Context, chat_id: int | str, query: str, limit: int = 20
    ) -> dict:
        """在聊天内搜索消息(按相关性,新→旧)。"""
        state: ServerState = ctx.request_context.lifespan_context
        access = await access_for(ctx, state)
    ...
    @mcp.tool()
    @wrap_errors
    async def get_chat_members_count(ctx: Context, chat_id: int | str) -> dict:
        """群/频道成员数(join/leave 测试后断言成员数变化)。"""
        state: ServerState = ctx.request_context.lifespan_context
        access = await access_for(ctx, state)
        chat_id = await resolve_chat_id(state.client, chat_id)
        require_chat(access, chat_id)
        return await state.client.get_chat_members_count(chat_id)
